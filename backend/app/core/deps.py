...
async def get_current_customer_user(
    ...
    """Get the current user and verify they have CUSTOMER role.
    ...
    Returns:
        The authenticated User ORM instance with CUSTOMER role.

    Raises:
        HTTPException 401: If the token is invalid or the user is not found.
        HTTPException 403: If the user does not have CUSTOMER role.
    """
    user = await get_current_user(token=token, db=db)
